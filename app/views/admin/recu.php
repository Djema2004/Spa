<?php
// 1. Tcheke si Controller a te deja voye done yo anndan $rendezvous
if (!isset($rendezvous) || empty($rendezvous)) {
    
    // Koneksyon ak baz de done a si li pa te chaje depi nan controller
    ...
    if (!$db && class_exists('Connect')) {
        $dbConnect = new Connect();
        $db = $dbConnect->pdo ?? null;
    }
        ...
        try {
            $stmt = $db->prepare("
                SELECT 
                    ...
                    r.service,
                    CONCAT(e.prenom, ' ', e.nom) AS estheticienne,
                    r.montant,
    ...
    // Si done yo te soti dirèkteman nan Controller a
    $rdv = [
        'id' => $rendezvous['id_rendezvous'] ?? $rendezvous['id'] ?? 1,
        'client' => $rendezvous['client_nom'] ?? $rendezvous['client'] ?? 'Kliyan',
        'client_tel' => $rendezvous['client_phone'] ?? $rendezvous['client_tel'] ?? 'N/A',
        'client_email' => $rendezvous['client_email'] ?? '',
        'service' => $rendezvous['service'] ?? 'N/A',
        'estheticienne' => $rendezvous['estheticienne_nom'] ?? $rendezvous['estheticienne'] ?? 'N/A',
        'montant' => $rendezvous['montant'] ?? 0,
        'mode' => $rendezvous['mode_paiement'] ?? $rendezvous['mode'] ?? 'Kès',
        'date' => $rendezvous['date_rendezvous'] ?? $rendezvous['date'] ?? date('Y-m-d H:i:s'),
        'statut' => $rendezvous['statut'] ?? 'En attente'
    ];
}

// Fallback si tout chèchè yo pa jwenn anyen anndan DB a
if (!$rdv) {
    $rdv = [
        'id' => $_GET['id'] ?? 1,
        'client' => 'Marie Jean',
...
<body class="min-h-screen flex flex-col items-center justify-center p-6">
    ...
    <div class="no-print mb-6 flex items-center gap-3">
        <a href="/spa/rendezvous" class="bg-white/80 hover:bg-white text-[#5C3A3C] px-4 py-2 rounded-xl text-sm font-semibold border border-[#F5E6E8] shadow-sm transition flex items-center gap-2">
